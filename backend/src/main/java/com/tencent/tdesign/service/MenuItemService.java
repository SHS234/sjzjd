...
public class MenuItemService {
  ...
  public List<RouteItem> getMenuRoutesForCurrentUser() {
    List<MenuItemEntity> items = filterFinance(menuItemMapper.selectAllEnabled());
  ...
  public List<MenuItemTreeNode> getAdminTree() {
    List<MenuItemEntity> items = filterFinance(menuItemMapper.selectAll());
    return toAdminTree(items);
  }

  private List<MenuItemEntity> filterFinance(List<MenuItemEntity> items) {
    if (items == null || items.isEmpty()) return items;
    List<MenuItemEntity> out = new ArrayList<>();
    for (MenuItemEntity e : items) {
      if (containsFinance(e.getRouteName())) continue;
      if (containsFinance(e.getPath())) continue;
      if (containsFinance(e.getTitleZhCn())) continue;
      if (containsFinance(e.getTitleEnUs())) continue;
      out.add(e);
    }
    return out;
  }

  private boolean containsFinance(String value) {
    if (value == null || value.isBlank()) return false;
    String v = value.toLowerCase(Locale.ROOT);
    return v.contains("finance") || v.contains("财务");
  }
  ...
  @Transactional
  public boolean removeObsoleteOrderHallRoute() {
    MenuItemEntity page = menuItemMapper.selectByRouteName("OrderHall");
    if (page != null) return delete(page.getId(), true);
    return false;
  }
  ...
  private RouteItem toRoute(MenuItemTreeNode n) {
    ...
    // Special handling for OrderRoot to show as single item
    if ("OrderRoot".equals(n.getRouteName())) {
      meta.setSingle(true);
    }
