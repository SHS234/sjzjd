...
public class SecuritySettingService {
  ...
  public boolean applyRequest(UiSettingRequest req) {
    ...
    if (token == null) token = new SecurityTokenSetting();
    ...
    if (captcha == null) captcha = new SecurityCaptchaSetting();
    ...
    if (policy == null) policy = new SecurityPasswordPolicy();
  ...
  private void upsertToken(SecurityTokenSetting setting) {
    if (setting.getId() == null) tokenMapper.insert(setting);
    else tokenMapper.update(setting);
  }

  private void upsertCaptcha(SecurityCaptchaSetting setting) {
    if (setting.getId() == null) captchaMapper.insert(setting);
    else captchaMapper.update(setting);
  }

  private void upsertPasswordPolicy(SecurityPasswordPolicy policy) {
    if (policy.getId() == null) passwordMapper.insert(policy);
    else passwordMapper.update(policy);
  }
}
