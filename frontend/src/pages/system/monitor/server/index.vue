      ...
      <t-col :xs="24" :sm="12" :md="6">
        <t-card title="内存" :bordered="false">
          <t-descriptions :column="2" layout="horizontal">
            ...
            <t-descriptions-item label="JVM">{{ serverInfo.mem?.jvmTotal || '-' }}</t-descriptions-item>
            <t-descriptions-item label="已用内存">{{ serverInfo.mem?.used || '-' }}</t-descriptions-item>
            <t-descriptions-item label="">{{ serverInfo.mem?.jvmUsed || '-' }}</t-descriptions-item>
            <t-descriptions-item label="剩余内存">{{ serverInfo.mem?.free || '-' }}</t-descriptions-item>
            <t-descriptions-item label="">{{ serverInfo.mem?.jvmFree || '-' }}</t-descriptions-item>
            <t-descriptions-item label="使用率">{{ serverInfo.mem?.usage || '-' }}</t-descriptions-item>
            <t-descriptions-item label="">{{ serverInfo.mem?.jvmUsage || '-' }}</t-descriptions-item>
          </t-descriptions>
        </t-card>
      </t-col>
    </t-row>
