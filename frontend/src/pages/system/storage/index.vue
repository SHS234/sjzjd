<template>
  <t-card title="对象存储" :bordered="false">
    <t-space direction="vertical" size="large" style="width: 100%">
      <t-alert
        theme="info"
        message="可在此配置系统上传能力，支持本地存储、阿里云 OSS、腾讯云 COS。保存后新的上传会自动按最新配置执行。"
        :close="false"
      />
      <t-form ref="formRef" :data="form" :rules="rules" layout="vertical" label-align="right" label-width="140px" colon>
        <t-row :gutter="[24, 16]">
          ...
          <t-col :xs="24" :sm="12">
            <t-form-item label="命名空间/桶" name="bucket" help="阿里云 Bucket 或腾讯云存储桶名称">
              <t-input
                v-model="form.bucket"
          ...
          <t-col :xs="24" :sm="12">
            <t-form-item label="所属 Region" name="region" help="腾讯云必填，阿里云可留空">
              <t-input
                v-model="form.region"
                :disabled="!canEdit || form.provider === 'LOCAL'"
                style="max-width: 500px; width: 100%"
          ...
          <t-col :xs="24" :sm="12">
            <t-form-item label="Endpoint" name="endpoint" help="阿里云、腾讯云私有域名可在此指定">
              <t-input
