...
const uploadTips =
  '请选择 Office/PDF/图片等常见格式。编辑时可保留当前文件，如需替换请先移除再重新上传。文件会先缓存在本地，点击提交后开始分片上传并支持断点续传，建议单文件大小 10GB 内。';
...
:deep(.download-preview-dialog .t-dialog__body) {
  padding-bottom: 0;
  overflow: hidden;
}

:deep(.download-preview-dialog .t-dialog__footer) {
  padding: 6px 16px;
}
</style>
