<template>
  ...
  >
    ...
    <t-form-item name="idCard">
      <t-input v-model="formData.idCard" :maxlength="18" size="large" placeholder="请输入身份证号">
        <template #prefix-icon>
          <t-icon name="card" />
        </template>
      </t-input>
...
const FORM_RULES: Record<string, FormRule[]> = {
  ...
  idCard: [
    { required: true, message: '请输入身份证号', type: 'error' as const },
    {
      validator: (val) => !val || /^\d{15}$|^\d{17}[\dx]$/i.test(String(val)),
      message: '身份证号格式不正确',
      type: 'error' as const,
    },
  ],
...
const formRules = computed(() => ({
  ...FORM_RULES,
  captcha: captchaType.value === 'image' ? [{ required: true, message: '请输入验证码', type: 'error' as const }] : [],
}));
