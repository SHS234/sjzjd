  ...
  actions: {
    ...
    async refreshAsyncRoutes(_userInfo?: UserInfo) {
      try {
        ...
        // 在权限刷新时同步标签页路由信息，但避免递归更新
        tabsRouterStore.syncTabPathByRouteName((tab) => {
          if (!tab.name || !router.hasRoute(tab.name)) return null;
          try {
            const resolved = router.resolve({ name: tab.name, query: tab.query });
            // Keep `tab.path` as a pure path (query is stored separately on the tab),
            // otherwise Tabs `value` can desync and cause recursive updates.
