...
const debouncedSave = debounce(async (data) => {
  ...
  try {
    await settingStore.saveUiSetting(data);
  } catch (e) {
...
watch(
  ...
  (value) => {
    ...
    settingStore.updateConfig(value);
    debouncedSave(value);
  },
  { deep: true },
);
...
watch(
  ...
  () => {
    syncing.value = true;
    const keys = Object.keys(STYLE_CONFIG) as Array<keyof typeof STYLE_CONFIG>;
    keys.forEach((key) => {
      (formData.value as any)[key] = settingStore[key];
    });
    nextTick(() => {
      syncing.value = false;
    });
